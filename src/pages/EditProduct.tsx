...
const nonEmptyString = z.union([z.string(), z.number()])
    ...
    const fetchProduct = async () => {
      ...
      try {
        const res = await fetch(
          `/api/macros/s/AKfycbzCfR-b6Nxb_foHkE2SlJR_c4uPRm_CjCtsX64UoQmdacbftdNS-NM8nY6CJok0m0CBtQ/exec?action=product&productid=${productId}`
        );
...
console.log("Payload to send:", payload);
    try {
      const res = await fetch(
        `/api/macros/s/AKfycbxMf0T7Y1ilUXrR3OE_H8YGHFnPwOe-Yvu1fJkOfrz7t35INT94uBtqlXRorUmh6iMPwQ/exec`,
        {
          method: "POST",
           headers: {
...
>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField name="specification" control={form.control} render={({ field }) => (
                  <FormItem>
...
<Controller name="category" control={form.control} render={({ field }) => (
  <FormItem>
    ...
    <ReactSelect
      ...
      value={
        categories
          .map(c => ({ value: String(c.Category_ID), label: c.Category_Name || c.categoryName }))
  ...
  <FormItem>
    ...
    <ReactSelect
      ...
      value={
        suppliers
          .map(s => ({ value: String(s.Supplier_ID), label: s.Supplier_Name }))
          .find(opt => opt.value === String(field.value))
      }
      onChange={opt => field.onChange(opt?.value)}
      isSearchable
  ...
  <FormItem>
    ...
    <ReactSelect
      options={warehouses.map(w => ({
        ...
        label: `${w.Warehouse_Name}${w.Warehouse_Location ? ` (${w.Warehouse_Location})` : ""}`,
      }))}
      value={
        warehouses
          .map(w => ({
...
<FormField control={form.control} name="Margin" render={({ field }) => (
                ...
                <FormField name="EANCode" control={form.control} render={({ field }) => (
                  <FormItem>
                ...
                <FormField name="ShortCode" control={form.control} render={({ field }) => (
                  <FormItem>
                ...
                <FormField name="ProductFamilyCode" control={form.control} render={({ field }) => (
                  <FormItem>
                ...
                <FormField name="entryDate" control={form.control} render={({ field }) => (
